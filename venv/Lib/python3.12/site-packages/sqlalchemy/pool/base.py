        ...
        try:
            ...
            if pool.dispatch.first_connect:
                pool.dispatch.first_connect.for_modify(
                    pool.dispatch
        ***REMOVED***.exec_once_unless_exception(self.dbapi_connection, self)

            # init of the dialect now takes place within the connect
            # event, so ensure a mutex is used on the first run
            pool.dispatch.connect.for_modify(
                pool.dispatch
    ***REMOVED***._exec_w_sync_on_first_run(self.dbapi_connection, self)
...
***REMOVED***, "can't 'checkout' a detached connection fairy"
        assert (
            fairy.dbapi_connection is not None
***REMOVED***, "can't 'checkout' an invalidated connection fairy"

        fairy._counter += 1
        if (
            not pool.dispatch.checkout and not pool._pre_ping
***REMOVED*** or fairy._counter != 1:
            return fairy
