...
***REMOVED***.get_moving_window(item.key_for(*identifiers), item.amount, item.get_expiry())
...
***REMOVED***.acquire_sliding_window_entry(
            item.key_for(*identifiers),
            item.amount,
            item.get_expiry(),
            cost,
***REMOVED***
    ...
    async def test(self, item: RateLimitItem, *identifiers: str, cost: int = 1) -> bool:
        ...
        previous_count, previous_expires_in, current_count, _ = await cast(
            SlidingWindowCounterSupport, self.storage
***REMOVED***.get_sliding_window(item.key_for(*identifiers), item.get_expiry())
...
***REMOVED***
    ...
    async def get_window_stats(
        ...
        (
            ...
            current_count,
            current_expires_in,
***REMOVED*** = await cast(SlidingWindowCounterSupport, self.storage).get_sliding_window(
...
***REMOVED***
    ...
    async def clear(self, item: RateLimitItem, *identifiers: str) -> None:
        return await cast(
            SlidingWindowCounterSupport, self.storage
***REMOVED***.clear_sliding_window(item.key_for(*identifiers), item.get_expiry())
